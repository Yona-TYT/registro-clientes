...
function create_table_servicios(){

	//----------------------------------------------------------------
	//Nombre de las celdas value--------------------------------------
	var name_cel = ["Nombre", "Descripcion", "Costo", "Accion"];
	var name_siz = name_cel.length;
	//----------------------------------------------------------------

	var sect_table = document.getElementById("sect_rs");

	sect_table.innerHTML = "";

	var tabla = document.createElement("table");
	tabla.setAttribute("id", "table_rs");
	...
	for (var j = 0; j < siz_fil; j++) {
		...
		for (var i = 0; i < siz_col; i++) {
			...
			if(siz_f==0){
				...
				celda.setAttribute("id", "celdrs"+celda_id)
				celda.setAttribute("class","celda_style");

				// Creamos 2 elementos de entrada
				var input = document.createElement("input");
				input.setAttribute("id", "inputrs"+celda_id);
				input.setAttribute("type", "number");
				var tex_mask = document.createElement("input");
				input.setAttribute("readonly", "");
			...
			//--------------------------------------------------------------------------------------------------
				...
				if(siz_c==2 ){
					...
					//para la mask del cuadro
					tex_mask.setAttribute("id", "text_maskrs"+celda_id);
					celda.appendChild(tex_mask);
					celda.appendChild(input);
...
function create_table_pagos(){
	...
	//----------------------------------------------------------------
	//Nombre de las celdas value--------------------------------------
	var name_cel = ["Descripcion", "Costo", "Nr. Veces", "Accion"];
	var name_siz = name_cel.length;
	//----------------------------------------------------------------
		...
		for (var i = 0; i < siz_col; i++) {
			...
			//--------------------------------------------------------------------------------------------------
				...
				if(siz_c==3){
					...
					button.setAttribute("onclick","add_service();");
