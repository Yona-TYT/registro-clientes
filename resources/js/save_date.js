...
function crearbd(evento) {
	...
	var alma_depositos = basededatos.createObjectStore("productos", {keyPath:"id", autoIncrement: true});
	alma_depositos.createIndex("buscarnombre", "nombre", {unique: true});

	//Guarda los datos de cada Cliente
	var alma_clientes = basededatos.createObjectStore("clientes", {keyPath:"id", autoIncrement: true});
	alma_clientes.createIndex("buscarnombre", "nombre", {unique: true});

	//Guarda los Captures de cada pago
	var alma_capture = basededatos.createObjectStore("servicios", {keyPath:"id", autoIncrement: true});
...
function agregar_capture(datos,clave) {
	var transaccion = bd.transaction(["servicios"], "readwrite");
	var almacen = transaccion.objectStore("servicios");
...
function mostrar_cuentas(clave) {
	var transaccion = bd.transaction(["productos"]);
	var almacen = transaccion.objectStore("productos");
	var solicitud = almacen.get(clave);
...
function mostrar_captures(clave,name) {
	var transaccion = bd.transaction(["servicios"]);
	var almacen = transaccion.objectStore("servicios");
	var solicitud = almacen.get(clave);
